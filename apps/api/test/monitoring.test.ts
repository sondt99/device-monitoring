...
describe('monitoring service', () => {
  it('records status transitions as beats', async () => {
    ...
    migrate(db);
    const device = createDevice(db, { name: 'Router', host: '127.0.0.1', checkType: 'ping' as const, checkUrl: null, checkPort: null, intervalSeconds: 10, timeoutMs: 500, retries: 0, enabled: true });
